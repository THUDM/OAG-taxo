...
class UnifiedDataLoader(DataLoader):
    """
        Dataloader Used by Baseline model and Enrich Model
        Provide query node, parent node, parent path, children node, children path and sibling information
    """
    ...
    def __init__(self, mode, data_path, sampling_mode=1, batch_size=10, negative_size=20, max_pos_size=100,
        ...
        super(UnifiedDataLoader, self).__init__(dataset=self.dataset, batch_size=self.batch_size, shuffle=self.shuffle,
                                                ...
                                                pin_memory=False)
        # total number of samples that will be emitted by this data loader
        self.n_samples = len(self.dataset)

    def collate_fn(self, samples):
        if 'g' in self.mode and 'p' in self.mode:
            us, vs, ss, graphs_u, graphs_v, paths_u, paths_v, lens, queries, labels = map(
                list, zip(*chain(*samples)))
            lens = torch.tensor(lens)
            max_u, max_v = lens.max(dim=0)[0]
            paths_u = [p + [self.dataset.pseudo_leaf_node]
                       * (max_u - len(p)) for p in paths_u]
            paths_v = [p + [self.dataset.pseudo_leaf_node]
                       * (max_v - len(p)) for p in paths_v]
            sib_len = torch.tensor([len(s) for s in ss])
            sib_s = [None] * len(ss)
            for i in range(len(ss)):
                if len(ss[i]) < self.num_sib:
                    sib_s[i] = ss[i] + [self.dataset.pseudo_leaf_node] * \
                        (self.num_sib - len(ss[i]))
                else:
                    sib_s[i] = ss[i]
            if 's' in self.mode:
                return torch.tensor(queries), torch.tensor(labels), torch.tensor(us), torch.tensor(vs), torch.tensor(sib_s), \
                    graphs_u, graphs_v, torch.tensor(
                        paths_u), torch.tensor(paths_v), lens, sib_len
            else:
                return torch.tensor(queries), torch.tensor(labels), torch.tensor(us), torch.tensor(vs), \
                    graphs_u, graphs_v, torch.tensor(
                        paths_u), torch.tensor(paths_v), lens
        elif 'g' in self.mode:
            us, vs, ss, graphs_u, graphs_v, queries, labels = map(
                list, zip(*chain(*samples)))
            sib_len = torch.tensor([len(s) for s in ss])
            sib_s = [None] * len(ss)
            for i in range(len(ss)):
            ...
            us, vs, ss, paths_u, paths_v, lens, queries, labels = map(
                list, zip(*chain(*samples)))
            lens = torch.tensor(lens)
            sib_len = torch.tensor([len(s) for s in ss])
            sib_s = [None] * len(ss)
            ...
            if 's' in self.mode:
                return torch.tensor(queries), torch.tensor(labels), torch.tensor(us), torch.tensor(vs), torch.tensor(sib_s),\
                    None, None, torch.tensor(paths_u), torch.tensor(
                        paths_v), lens, sib_len
            else:
...
class TaxoExpanDataLoader(DataLoader):
    """
        Dataloader Used For Expan on Expan Task
        provide query node, parent node, parent egonet
    """
    ...
    def collate_fn(self, samples):
        us, graphs_u, paths_u, lens, queries, labels = map(
            list, zip(*chain(*samples)))
...
class TaxoExpanDataLoaderTMN(DataLoader):
    """
        Dataloader Used For Expan on Completion Task
        provide query node, parent node, parent egonet and children node
    """
    ...
    def __init__(self, mode, data_path, sampling_mode=1, batch_size=10, negative_size=20, max_pos_size=100,
        ...
        raw_graph_dataset = MAGDataset(name="", path=data_path, raw=False)
        msk_graph_dataset = ExpanDatasetTMN(raw_graph_dataset, sampling_mode=sampling_mode,
                                            negative_size=negative_size,
                                            max_pos_size=max_pos_size, expand_factor=expand_factor,
                                            cache_refresh_time=cache_refresh_time, normalize_embed=normalize_embed,
                                            test_topk=test_topk)
        self.dataset = msk_graph_dataset
        self.num_workers = num_workers
        super(TaxoExpanDataLoaderTMN, self).__init__(dataset=self.dataset, batch_size=self.batch_size, shuffle=self.shuffle,
                                                     collate_fn=self.collate_fn, num_workers=self.num_workers,
                                                     pin_memory=True)
        # total number of samples that will be emitted by this data loader
        self.n_samples = len(self.dataset)
    ...
    def collate_fn(self, samples):
        us, vs, graphs_u, paths_u, lens, queries, labels = map(
            list, zip(*chain(*samples)))
        if 'g' not in self.mode:
            graphs_u = None
        if 'r' in self.mode:
    ...
    def __str__(self):
        return "\n\t".join([
            f"TaxoExpanDataLoaderTMN mode: {self.mode}",
            f"sampling_mode: {self.sampling_mode}",
            f"batch_size: {self.batch_size}",
            f"negative_size: {self.negative_size}",
            f"expand_factor: {self.expand_factor}",
...
class TaxoExpanSequenceDataloader(DataLoader):
    def __init__(self, mode, data_path, sampling_mode=1, batch_size=10, negative_size=20, max_pos_size=100,
                 expand_factor=50, shuffle=True, num_workers=8, cache_refresh_time=64, normalize_embed=False,
                 test_topk=-1):
        self.mode = mode

...
MAX_CANDIDATE_NUM = 100


def rearrange(energy_scores, candidate_position_idx, true_position_idx):
    tmp = np.array([[x == y for x in candidate_position_idx]
                   for y in true_position_idx]).any(0)
    correct = np.where(tmp)[0]
    incorrect = np.where(~tmp)[0]
    labels = torch.cat(
        (torch.ones(len(correct)), torch.zeros(len(incorrect)))).int()
    energy_scores = torch.cat(
        (energy_scores[correct], energy_scores[incorrect]))
...
class Trainer(BaseTrainer):
    ...
    def __init__(self, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        ...
        self.writer.add_text(
            'Text', 'Model Architecture: {}'.format(self.config['arch']), 0)
        self.writer.add_text('Text', 'Training Data Loader: {}'.format(
            self.config['train_data_loader']), 0)
        self.writer.add_text(
            'Text', 'Loss Function: {}'.format(self.config['loss']), 0)
        self.writer.add_text('Text', 'Optimizer: {}'.format(
            self.config['optimizer']), 0)
...
class TrainerS(Trainer):
    ...
    def __init__(self, mode, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        super(TrainerS, self).__init__(model, loss, metrics,
                                       pre_metric, optimizer, config, data_loader, lr_scheduler)
        self.mode = mode
        self.num_count = 0
        ...
        if len(self.candidate_positions) > MAX_CANDIDATE_NUM:
            valid_pos = set(itertools.chain.from_iterable(
                dataset.valid_node2pos.values()))
            valid_neg = list(
                set(self.candidate_positions).difference(valid_pos))
            valid_sample_size = max(MAX_CANDIDATE_NUM - len(valid_pos), 0)
            self.valid_candidate_positions = random.sample(
                valid_neg, valid_sample_size) + list(valid_pos)
        else:
        ...
        if 'g' in mode:
            ...
            self.edge2subgraph = {e: dataset._get_subgraph_and_node_pair(
                -1, e[0], e[1]) for e in tqdm(self.candidate_positions, desc='collecting nodegraph')}
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            if self.is_infonce_training:
                ...
                target = torch.zeros(
                    n_batches, dtype=torch.long).to(self.device)
    ...
    def _test(self, mode, gpu=True):
        ...
        with torch.no_grad():
            ...
            if mode == 'test':
                ...
                self.logger.info(
                    f'number of candidate positions: {len(candidate_positions)}')
            else:
            ...
            for edges in tqdm(mit.sliced(candidate_positions, batch_size), desc="Generating graph encoding ..."):
                ...
                if 'g' in self.mode:
                    ...
                    bgu, bgv = zip(*bgs)
                if 'p' in self.mode:
                    bpu, bpv, bsibling, lens, sib_len = dataset._get_batch_edge_node_path(
                        edges)
                    bpu = bpu
                    bpv = bpv
                    sib = bsibling
                    lens = lens
                    sib_len = sib_len
                ur, vr, sr = self.model.forward_encoders(
                    us, vs, sib, bgu, bgv, bpu, bpv, lens, sib_len)
                hs, sib_len = sr
                batched_model.append((ur.detach().cpu(), vr.detach(
                ).cpu(), hs.detach().cpu(), sib_len.detach().cpu()))
            ...
            for i, query in tqdm(enumerate(vocab), desc='testing'):
                ...
                batched_energy_scores, labels = rearrange(
                    batched_energy_scores, candidate_positions, node2pos[query])
                all_ranks.extend(self.pre_metric(
                    batched_energy_scores, labels))
...
class TrainerB(TrainerS):
    ...
    def __init__(self, mode, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        super(TrainerB, self).__init__(mode, model, loss, metrics,
                                       pre_metric, optimizer, config, data_loader, lr_scheduler)
...
class TrainerT(TrainerS):
    ...
    def __init__(self, mode, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        super(TrainerT, self).__init__(mode, model, loss, metrics,
                                       pre_metric, optimizer, config, data_loader, lr_scheduler)
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            self.optimizer.zero_grad()
            scores, scores_p, scores_c, scores_e, scores_s = self.model(
                nf, u, v, sib, bgu, bgv, bpu, bpv, lens, sib_s)
            label = label.to(self.device)
            ...
            loss = self.loss(scores, label[:, 0]) + self.l1 * loss_p + \
                self.l2 * loss_c + self.l3 * loss_e + self.l4 * loss_s
            loss.backward()
            self.optimizer.step()
...
class TrainerTExpan(Trainer):
    ...
    def __init__(self, mode, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        super(TrainerTExpan, self).__init__(model, loss, metrics,
                                            pre_metric, optimizer, config, data_loader, lr_scheduler)
        self.mode = mode
        ...
        self.candidate_positions = list(set(
            [(p, c) for (p, c) in data_loader.dataset.all_edges if c == self.pseudo_leaf]))
        self.valid_node2pos = {node: set([(p, c) for (
            p, c) in pos_l if c == self.pseudo_leaf]) for node, pos_l in dataset.valid_node2pos.items()}
        self.test_node2pos = {node: set([(p, c) for (
            p, c) in pos_l if c == self.pseudo_leaf]) for node, pos_l in dataset.test_node2pos.items()}
        self.valid_vocab = [node for node,
                            pos in self.valid_node2pos.items() if len(pos)]
        self.test_vocab = [node for node,
    ...
    def _train_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.data_loader):
            ...
            self.optimizer.zero_grad()
            scores, scores_p, scores_c, scores_e = self.model(
                nf, u, v, bgu, bgv, bpu, bpv, lens)
            label = label.to(self.device)
            loss_p = self.loss(scores_p, label[:, 1])
            loss_c = self.loss(scores_c, label[:, 2])
            loss_e = self.loss(scores_e, label[:, 0])
            loss = self.loss(
                scores, label[:, 0]) + self.l1*loss_p + self.l2*loss_c + self.l3*loss_e
            loss.backward()
            self.optimizer.step()
    ...
    def _test(self, mode, gpu=True):
        ...
        with torch.no_grad():
            ...
            for edges in tqdm(mit.sliced(candidate_positions, batch_size), desc="Generating graph encoding ..."):
                ...
                ur, vr = self.model.forward_encoders(
                    us, vs, bgu, bgv, bpu, bpv, lens)
                batched_model.append((ur.detach().cpu(), vr.detach().cpu()))
                batched_positions.append(len(edges))
...
class TrainerExpan(Trainer):
    ...
    def __init__(self, mode, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        super(TrainerExpan, self).__init__(model, loss, metrics,
                                           pre_metric, optimizer, config, data_loader, lr_scheduler)
        self.mode = mode
        ...
        if 'g' in mode:
            self.all_nodes = sorted(list(dataset.core_subgraph.nodes))
            self.node2subgraph = {node: dataset._get_subgraph_and_node_pair(
                -1, node) for node in tqdm(self.all_nodes, desc='collecting nodegraph')}
...
class TrainerExpanTMN(Trainer):
    ...
    def __init__(self, mode, model, loss, metrics, pre_metric, optimizer, config, data_loader, lr_scheduler=None):
        super(TrainerExpanTMN, self).__init__(model, loss, metrics,
                                              pre_metric, optimizer, config, data_loader, lr_scheduler)
        self.mode = mode
...
class TrainerExpanSequence(Trainer):
    """

...
class AttnBIM(nn.Module):
    def __init__(self, l_dim, r_dim):
        super(AttnBIM, self).__init__()
        self.W = nn.Bilinear(l_dim * 2 + r_dim, r_dim, 1, bias=False)
        
    def forward(self, u, v, q, h):
        """
        e1: tensor of size (*, l_dim)
        e2: tensor of size (*, r_dim)
        ...
        e = torch.cat((u, v, q), -1)
        k = h.shape[-2]
        return self.W(e.unsqueeze(-2).repeat_interleave(k, -2), h)  # bxk
...
class Enrich(nn.Module):
    ...
    # def __init__(self, l_dim, r_dim, k=5, non_linear=nn.Tanh()):
        ...
        self.u = nn.Linear(k*4, 1, bias=False)
        self.u_l = nn.Linear(k, 1, bias=False)
        self.u_r = nn.Linear(k, 1, bias=False)
        self.u_e = nn.Linear(k, 1, bias=False)
        self.u_s = nn.Linear(k, 1, bias=False)
        ...
        self.W = nn.Bilinear(l_dim*2+r_dim, r_dim, k, bias=True)
        self.V_l = nn.Linear(l_dim + r_dim, k, bias=False)
        self.V_r = nn.Linear(l_dim + r_dim, k, bias=False)
        self.V = nn.Linear(l_dim*2 + r_dim*2, k, bias=False)
        self.W_s = nn.Bilinear(r_dim, r_dim, k, bias=True)
        self.V_s = nn.Linear(r_dim + r_dim, k, bias=False)

        self.control = nn.Sequential(nn.Linear(l_dim*2+r_dim*2, l_dim*2+r_dim, bias=False), nn.Sigmoid())
        ...
        self.control_s = nn.Sequential(nn.Linear(2*r_dim, r_dim, bias=False), nn.Sigmoid())

    def forward(self, e1, e2, sib, q):
        ec1 = e1 * self.control_l(torch.cat((e1, q), -1))
        ec2 = e2 * self.control_r(torch.cat((e2, q), -1))
        ec3 = sib * self.control_s(torch.cat((sib, q), -1))
        e = torch.cat((e1, e2, sib), 1)
        ec = e * self.control(torch.cat((e, q), -1))
        l = self.W_l(ec1, q) + self.V_l(torch.cat((ec1, q), 1))
        r = self.W_r(ec2, q) + self.V_r(torch.cat((ec2, q), 1))
        s = self.W_s(ec3, q) + self.V_s(torch.cat((ec3, q), 1))
        e = self.W(ec, q) + self.V(torch.cat((ec, q), 1))
        l_scores = self.u_l(self.f(l))
        r_scores = self.u_r(self.f(r))
        e_scores = self.u_e(self.f(e))
        s_scores = self.u_s(self.f(s))
        scores = self.u(self.f(torch.cat((e.detach(), l.detach(), r.detach(), s.detach()), -1)))
        if self.training:
            return scores, l_scores, r_scores, e_scores, s_scores
        else:
            return scores

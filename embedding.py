    ...
    for i in range(len(parents)):
        ...
        for j in range(len(ascendants)):
            if j != len(ascendants) - 1:
                sentence += taxoord2term[int(ascendants[j])] + ", "
            else:
                sentence += taxoord2term[int(ascendants[j])]
                sentence += " is a super class of {}".format(taxoord2term[int(node)])
    ...
    for i in range(len(children)):
        ...
        for j in range(len(descendants)):
            if j == len(descendants) - 1:
                sentence += taxoord2term[int(descendants[j])]
                sentence += " is a subclass of {}".format(taxoord2term[int(node)])
            else:
...
def phrase_emb_extract(sentence: str, phrase_list: List[str], model, tokenizer, device='cuda:0') -> List[torch.tensor]:
    emb_list = list()
...
def generate_embeddings(taxoid2sent, node, taxoord2term,model, tokenizer):
    ...
    if len(parent_sentence) == 0:
        ...
        parent_embeddings = torch.mean(
            torch.stack([phrase_emb_extract(i, [taxoord2term[int(node)]], model, tokenizer) for i in parent_sentence]), dim=0)
    if len(children_sentence) == 0:
        children_embeddings = torch.zeros(768)
    else:
        children_embeddings = torch.mean(
            torch.stack([phrase_emb_extract(i, [taxoord2term[int(node)]], model, tokenizer) for i in children_sentence]), dim=0)
...
def main():
    ...
    taxoord2term = {}
    ord = 0
    with open('./data/Data/Artificial Intelligence.terms', 'r') as f:
        for lines in f:
            line = lines.strip()
            if line:
                ...
                ord += 1
    taxonomy = nx.DiGraph()
    with open('./data/Data/Artificial Intelligence.taxo', 'r') as f:
        for lines in f:
            line = lines.strip()
            if line:
    ...
    root_path = _get_path_to_root(taxonomy)
    leaf_path = _get_path_to_leaf(taxonomy)

    taxoid2sent = {}
    nodes = list(taxonomy.nodes)
    # print(taxoord2term[681])
    for i in tqdm(range(len(nodes)), desc="Generating sentences"):
        parent_sentence, children_sentence = generate_sentences(root_path, leaf_path, taxonomy, nodes[i],
                                                                taxoord2term)
    ...
    for i in tqdm(range(len(nodes)), desc="Loading embeddings"):
        id = taxoord2id[int(nodes[i])]
        taxoid2emb[id] = generate_embeddings(taxoid2sent, nodes[i], taxoord2term, model, tokenizer)

    with open('data/Data/Artificial Intelligence.terms.embed', 'w') as f:
    ...
    with open('./data/Data/Artificial Intelligence.terms.bertembed', 'w') as f:
